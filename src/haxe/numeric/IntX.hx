...
abstract IntX(Array<Int>) {
	 ...
	 */
	static public function createBits(value:Int, n:Int):IntX {
		...
		for(i in 0...n) {
			if(v == 0) {
				...
				data[i] = v & Numeric.native32BitsInt;
				v = (v >>> 31) >>> 1;
			}
		}
			...
			if(value != 0) {
				throw new OverflowException('$value overflows Int${n*32}');
			}
		#end

		return new IntX(data);
	}
	...
	inline function new(data:Array<Int>) {
		this = data;
	}
	...
	public function toString():String {
		if(this.high == 0) {
			inline function normalized() {
				#if java
					return (cast (UInt32.MAX_AS_FLOAT + this + 1):java.lang.Number).longValue();
				#elseif lua
					return untyped __lua__('4294967295') + this + 1;
				#else
					return UInt32.MAX_AS_FLOAT + this.low + 1;
				#end
			}
			return this.low < 0 ? '${normalized()}' : '$this';
		}
